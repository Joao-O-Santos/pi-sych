...
try {
	...
	while (true) {
		...
		if (parent === current)
			throw new Error(
				"pi-mcporter has no resolvable compatible MCPorter runtime",
			);
		current = parent;
	}
	...
	const mcporterVersion = await readVersion(
		resolve(mcporterRoot, "package.json"),
	);
	const tree = execFileSync(
		"npm",
		["ls", "pi-mcporter", "mcporter", "--all", "--json"],
		{ encoding: "utf8" },
	);
