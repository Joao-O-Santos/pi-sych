...
| Public userland | Packaged public documentation, including the [v7 literature migration guide](literature-database-v7.md), this contract, `COPYING.md`, `docs/LICENSE.md`, and notices in `docs/LICENSES/`. | These documents and notices remain available in the package. |
| Compatibility-sensitive internal | Model-facing supervisor, worker, compaction, capability-summary, and persisted-session protocols. | Not a TypeScript API. Changes still need migration analysis, regression tests, and release notes when sessions or model behavior may be affected. The capability summary is derived, compact, and non-authorizing; integration inspection does not verify access. Custom compaction uses bounded observable messages, retained tail, and canonical snapshots, preserves recorded label attributions without verification, appends only bounded unreviewed inbox proposals, never mutates canonical semantic files, and returns native fallback on omission/failure. |
