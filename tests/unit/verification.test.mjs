...
test("verification executes exact argument arrays and records actual failure", async () => {
	...
	const failed = await runVerification(
		{
			executable: process.execPath,
			args: ["-e", "console.error('actual failure'); process.exit(3)"],
			expectedExitCode: 0,
		},
		root,
	);
	...
	assert.equal(
		validateVerificationCommand({ executable: "echo", args: [] })
			.expectedExitCode,
		0,
	);
	await assert.rejects(
		async () => runVerification({ executable: "echo", args: "bad" }, root),
...
test("verification contracts keep commands separate and retain all observed reports", async () => {
	...
	assert.deepEqual(
		reports.map((report) => report.exitCode),
		[0, 2],
	);
});
