...
test("worker capability modes expose exactly the intended built-in tools", () => {
  ...
  assert.deepEqual(toolsForMode("edit"), ["read", "grep", "find", "ls", "edit", "write", "submit_artifact"]);
  assert.deepEqual(toolsForMode("full-host"), ["read", "grep", "find", "ls", "edit", "write", "bash", "submit_artifact"]);
  ...
  assert.deepEqual(toolsForRequest({ mode: "read-only", remoteResearch: true }), ["read", "grep", "find", "ls", "submit_artifact", "mcporter"]);
  assert.deepEqual(toolsForRequest({ mode: "edit", remoteResearch: false }), toolsForMode("edit"));
});
...
test("selected skill names resolve to package skills from an external project", async () => {
  ...
  assert.deepEqual(resolveSelectedSkillPaths(["software-project", "local-review"], root, PI_SYCH_PACKAGE_ROOT), [join(PI_SYCH_PACKAGE_ROOT, "skills/software-project/SKILL.md"), join(projectSkill, "SKILL.md")]);
  assert.throws(() => resolveSelectedSkillPaths(["missing-skill"], root, PI_SYCH_PACKAGE_ROOT), /unavailable/);
});
...
test("dispatch schema validates bounded requests and model fallback is limited", async () => {
  ...
  assert.throws(() => validateDispatchRequest({ ...request, mode: "sandboxed" }), /Unknown worker mode/);
  assert.throws(() => validateDispatchRequest({ ...request, timeoutMs: 0 }), /positive integer/);
  assert.throws(() => validateDispatchRequest({ ...request, remoteResearch: "yes" }), /boolean/);
  const command = { executable: "node", args: ["-e", "", "-e"], expectedExitCode: 0 };
  assert.deepEqual(validateDispatchRequest({ ...request, verification: { commands: [command] } }).verification.commands[0].args, command.args);
  assert.deepEqual(resolveModelProfile({ default: ["one", "two", "three"] }), ["one", "two", "three"]);
  assert.deepEqual(resolveModelProfile({ default: ["one"], profiles: { review: ["review-one", "review-two"] } }, "review"), ["review-one", "review-two"]);
  assert.throws(() => resolveModelProfile({ default: [] }), /unavailable/);
  const root = await mkdtemp(join(tmpdir(), "pi-sych-missing-input-"));
  await assert.rejects(dispatchWorker({ projectRoot: root, workerAgentDir: join(root, "agent"), request: { ...request, inputs: [{ path: "missing.md", purpose: "required" }] }, profiles: { default: ["model"] }, launcher: async () => ({ exitCode: 0, stdout: "", stderr: "", timedOut: false }) }), /Selected input is unavailable/);
});

test("task prompt carries only the bounded packet and exact selected resources", () => {
  const verification = {
    commands: [{ executable: "npm", args: ["test", "--", "unit"], cwd: "repo", expectedExitCode: 0 }],
  };
  ...
  assert.match(prompt, /Supervisor verification contract \(runs after artifact submission\):/);
  ...
  const researchPrompt = taskPrompt({
    taskId: "task",
    runId: "run",
    request: validateDispatchRequest({ ...request, remoteResearch: true, reviewLens: "provenance" }),
    workerAgentDir: "/tmp/agent",
    resultPath: "/tmp/result",
    projectRoot: "/tmp/project",
    model: "model",
...
test("immutable worker submission and the project mutation lock preserve ownership", async () => {
  ...
  await assert.rejects(writeImmutableResult(resultPath, resultFor({ ...identity, request })), /immutable/);
  ...
  await assert.rejects(second.acquire({ taskId: "other", runId: "run" }), /already active/);
  await assert.rejects(second.release({ taskId: "other", runId: "run" }), /ownership/);
...
test("worker result validation rejects generated malformed verification reports", () => {
  ...
  const base = { ...resultFor({ ...identity, request }), taskId: identity.taskId, runId: identity.runId };
  assert.deepEqual(validateWorkerResult({ ...base, verification: [validReport] }, identity).verification, [validReport]);
  ...
  for (const report of malformed) {
    assert.throws(() => validateWorkerResult({ ...base, verification: [report] }, identity));
  }
});

test("real worker launcher resolves the package extension outside the Pi Sych repository", async () => {
  const projectRoot = await mkdtemp(join(tmpdir(), "pi-sych-external-project-"));
  ...
  try {
    const outcome = await launchPiWorker({ ...identity, request: validateDispatchRequest(request), workerAgentDir, resultPath, projectRoot, model: "test-model", prompt: "external task" });
...
test("remote worker receives exactly the MCPorter extension and explicit config", async () => {
  ...
  process.env.PI_SYCH_PI_BIN = fakePi;
  try {
    await launchPiWorker({ taskId: "remote-task", runId: "remote-run", request: validateDispatchRequest({ ...request, remoteResearch: true }), workerAgentDir: join(projectRoot, "agent"), resultPath: join(projectRoot, "result.json"), projectRoot, model: "test-model", prompt: "remote", packageRoot: PI_SYCH_PACKAGE_ROOT, extraExtensionPaths: [join(PI_SYCH_PACKAGE_ROOT, "node_modules/pi-mcporter/dist/index.js")] });
    const launch = JSON.parse(await readFile(argsPath, "utf8"));
    assert.deepEqual(launch.args.flatMap((item, index, args) => item === "--extension" ? [args[index + 1]] : []), [join(PI_SYCH_PACKAGE_ROOT, "extensions/worker/index.ts"), join(PI_SYCH_PACKAGE_ROOT, "node_modules/pi-mcporter/dist/index.js")]);
    assert.equal(launch.config, join(process.env.HOME, ".config/pi-sych/mcp/mcporter.json"));
...
test("real worker launcher terminates a cancelled external worker", async () => {
  const projectRoot = await mkdtemp(join(tmpdir(), "pi-sych-cancelled-project-"));
  const fakePi = join(projectRoot, "fake-pi-cancel");
  await writeFile(fakePi, "#!/usr/bin/env node\\nprocess.on('SIGTERM', () => process.exit(143));\\nsetInterval(() => {}, 1000);\\n");
  await chmod(fakePi, 0o755);
  const controller = new AbortController();
  const promise = launchPiWorker({ taskId: "cancel-task", runId: "cancel-run", request: validateDispatchRequest({ ...request, timeoutMs: 5000 }), workerAgentDir: join(projectRoot, "agent"), resultPath: join(projectRoot, "result.json"), projectRoot, model: "test-model", prompt: "cancel", signal: controller.signal });
...
test("dispatch returns an immutable result even when the worker process exits nonzero", async () => {
  ...
  const outcome = await dispatchWorker({
    ...
    launcher: async (spec) => {
      ...
      return { exitCode: 1, stdout: "partial output", stderr: "tool ended", timedOut: false };
    },
  });
...
test("dispatch runs only explicitly requested supervisor verification", async () => {
  ...
  const verified = await dispatchWorker({
    ...
    request: {
      ...request,
      verification: { commands: [{ executable: process.execPath, args: ["-e", "process.exit(0)"] }] },
    },
    profiles: { default: ["model"] },
    launcher,
  });
...
test("dispatch preserves the worker result and classifies failed explicit verification", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-sych-worker-verification-failure-"));
  await writeFile(join(root, "PROJECT.md"), "# Project\n");
  const outcome = await dispatchWorker({
    projectRoot: root,
    workerAgentDir: join(root, "agent"),
    request: {
      ...request,
      verification: { commands: [{ executable: process.execPath, args: ["-e", "process.exit(4)"], expectedExitCode: 0 }] },
    },
    profiles: { default: ["model"] },
    launcher: async (spec) => {
      await saveSyntheticWorkerResult(spec.resultPath, resultFor(spec));
...
test("dispatch classifies malformed worker results as schema failures", async () => {
  ...
  const outcome = await dispatchWorker({
    ...
    launcher: async (spec) => {
      await saveSyntheticWorkerResult(spec.resultPath, { ...resultFor(spec), status: "invalid" });
      return { exitCode: 0, stdout: "", stderr: "", timedOut: false };
    },
  });
...
test("model failures are classified from structured or terminal Pi errors", () => {
  assert.equal(classifyModelError("Error: model does not exist"), "model-unavailable");
  assert.equal(classifyModelError("HTTP 429: rate limit exceeded"), "model-limit");
  assert.equal(classifyModelError("worker wrote an invalid artifact"), undefined);
});

test("dispatch reports committed mutations and keeps runtime state Git-local", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-sych-worker-committed-change-"));
  ...
  const outcome = await dispatchWorker({
    ...
    profiles: { default: ["model"] },
    launcher: async (spec) => {
      await writeFile(join(root, "committed -> odd name.md"), "worker mutation\n");
  ...
  assert.deepEqual(outcome.result.observedChanges, ["committed -> odd name.md"]);
  assert.deepEqual(outcome.unexpectedChanges, ["committed -> odd name.md"]);
  assert.match(await readFile(join(root, ".git", "info", "exclude"), "utf8"), /^\.pi-sych\/$/m);
...
test("dispatch does not retry a transient failure after project mutation", async () => {
  ...
  const outcome = await dispatchWorker({
    ...
    launcher: async () => {
      ...
      return { exitCode: 1, stdout: "", stderr: "limited", timedOut: false, classification: "model-limit" };
    },
  });
...
test("dispatch retries only once for a retryable launch failure and reports scope differences", async () => {
  ...
  const outcome = await dispatchWorker({
    projectRoot: root,
    workerAgentDir: join(root, "agent"),
    request: { ...request, mode: "full-host", intendedWritePaths: ["expected.md"] },
    profiles: { default: ["first", "second", "third"] },
    launcher: async (spec) => {
      calls += 1;
      if (calls === 1) return { exitCode: 1, stdout: "", stderr: "unavailable", timedOut: false, classification: "model-unavailable" };
      await saveSyntheticWorkerResult(spec.resultPath, resultFor(spec));
      return { exitCode: 0, stdout: "", stderr: "", timedOut: false };
    },
  });
  ...
  assert.equal(outcome.result.status, "complete");
  assert.deepEqual(unexpectedChanges(["expected.md", "unexpected.md"], ["expected.md"]), ["unexpected.md"]);
});
