...
test("greenfield interview asks only its next focused question and leaves gaps unresolved", () => {
	assert.equal(
		nextGreenfieldQuestion({}),
		"What is the immediate objective of this project?",
	);
	assert.equal(
		nextGreenfieldQuestion({ objective: "Build a brief." }),
		"Who will use or evaluate the result?",
	);
	const candidate = buildGreenfieldCandidate({
		objective: "Build a brief.",
		audience: "A collaborator.",
	});
	assert.equal(candidate.requiresApproval, true);
	assert.equal(
		candidate.statements.find(
			(statement) => statement.group === "project objective",
		).label,
		"explicit",
	);
	assert.equal(
		candidate.statements.find(
			(statement) => statement.group === "intended contribution",
		).label,
		"unresolved",
	);
	assert.match(
		candidate.files.find((file) => file.path === "PROJECT.md").content,
		/\[explicit\] Build a brief\./,
	);
	assert.match(
		candidate.files.find((file) => file.path === "PROJECT.md").content,
		/\[unresolved\] Confirm intended contribution\./,
	);
});

test("existing manuscript produces observations, bounded inference, and open questions", async () => {
	const markdown = await readFile(
		"fixtures/stage3-manuscript/manuscript.md",
		"utf8",
	);
	const candidate = buildArtifactProjectCandidate("manuscript.md", markdown);
	assert.deepEqual(
		candidate.statements
			.filter((statement) => statement.label === "explicit")
			.map((statement) => statement.group),
		["artifact identity", "artifact structure"],
	);
	assert.equal(
		candidate.statements.find(
			(statement) => statement.group === "main artifact",
		).label,
		"inferred",
	);
	assert.equal(
		candidate.statements.find((statement) => statement.group === "objective")
			.label,
		"unresolved",
	);
	...
	assert.match(
		review,
		/No durable file will be written until explicit approval/,
	);
...
test("canonical files produce a coherent outline candidate with stated evidence and uncertainty", () => {
	const candidate = buildOutlineCandidate(
		"# Project\n\n## Objective\n\nA test\n",
		"# Evidence\n\n## E-001 — Support\n",
	);
	...
	assert.equal(
		candidate.statements.find(
			(statement) => statement.group === "artifact scope",
		).label,
		"unresolved",
	);
});
...
test("canonical files produce a bounded section candidate with no unsupported prose", () => {
	...
	assert.match(
		...
		/\[unresolved\] Add only claims supported/,
	);
	assert.match(
		formatCandidateReview(candidate),
		/Cite only relevant evidence entries: E-001/,
	);
});
...
test("candidate writes require approval and refuse project-root escape", async () => {
	...
	const candidate = buildGreenfieldCandidate({
		objective: "Only after approval.",
	});
	await assert.rejects(
		applyCandidate(root, candidate, false),
		/explicit approval/,
	);
	await assert.rejects(readFile(join(root, "PROJECT.md")), /ENOENT/);
	const written = await applyCandidate(root, candidate, true);
	assert.deepEqual(
		written.map((path) => path.slice(root.length + 1)),
		["PROJECT.md", "EVIDENCE.md"],
	);
	assert.match(
		await readFile(join(root, "PROJECT.md"), "utf8"),
		/Only after approval/,
	);
	await assert.rejects(
		applyCandidate(
			...
			{
				...candidate,
				files: [{ path: "../outside.md", content: "no", purpose: "bad" }],
			},
			true,
		),
...
test("sync candidate is reviewable and fingerprints observed existing files", async () => {
	...
	assert.match(
		formatCandidateReview(candidate),
		/Confirm each proposed authority domain/,
	);
});
