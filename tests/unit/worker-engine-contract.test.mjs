...
test("worker request and result retain the bounded protocol", () => {
	...
	assert.match(dispatchSchema.properties.skills.description, /skill selectors/);
	...
	assert.match(dispatchSchema.properties.mode.description, /not a sandbox/i);
	assert.deepEqual(
...
test("worker prompt carries assignment, context, persistence, and terminal-result boundaries", () => {
	...
	assert.match(research, /source validity, or completeness/);
	...
	assert.match(trajectory, /background for the explicit assignment/i);
