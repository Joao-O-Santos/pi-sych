...
test("model catalog rejects missing defaults and invalid aliases", () => {
  assert.throws(() => parseModelProfiles({ profiles: { code: ["provider/model"] } }), /profiles\.default/);
  assert.throws(() => parseModelProfiles({ models: { broken: {} }, profiles: { default: ["broken"] } }), /non-empty ref/);
});
