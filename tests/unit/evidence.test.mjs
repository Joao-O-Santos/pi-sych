...
import { fingerprintFile, inspectProjectSync } from "../../.test-build/workbench/src/sync.js";
...
test("evidence challenge detects missing sources and missing required fields", async () => {
  ...
  assert.equal(challenges.some((challenge) => challenge.entryId === "E-001" && challenge.issue === "missing-source"), true);
  assert.equal(challenges.some((challenge) => challenge.entryId === "E-002" && challenge.issue === "missing-field"), true);
});
...
test("evidence dependency state marks downstream prose for review when output changes", async () => {
  ...
  const fingerprints = await Promise.all(["EVIDENCE.md", "output.html", "manuscript.qmd"].map((path) => fingerprintFile(join(root, path))));
  await writeFile(join(root, "SYNC.md"), `# Project synchronization\n\n\`\`\`json\n${JSON.stringify({ version: 1, confirmedAt: "2026-07-28T12:00:00Z", artifacts: [
    { path: "EVIDENCE.md", role: "evidence", status: "current", authoritativeFor: ["claims"], fingerprint: fingerprints[0] },
    { path: "output.html", role: "analysis-output", status: "current", authoritativeFor: ["computed-results"], fingerprint: fingerprints[1] },
    { path: "manuscript.qmd", role: "main-artifact", status: "current", authoritativeFor: ["prose"], updateFrom: ["EVIDENCE.md", "output.html"], fingerprint: fingerprints[2] },
  ] }, null, 2)}\n\`\`\`\n`);
  ...
  assert.equal(await fingerprintEvidenceSource(root, "output.html"), state.artifacts.find((artifact) => artifact.path === "output.html").currentFingerprint);
});
