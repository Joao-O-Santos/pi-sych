...
function getCommands(args, env) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn("pi", args, { cwd: process.cwd(), env: { ...process.env, PI_OFFLINE: "1", ...env }, stdio: ["pipe", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
...
test("public workbench extension loads without private configuration", async () => {
  ...
  assert.deepEqual(names.filter((name) => name.startsWith("pi-sych-")).sort(), ["pi-sych-drift", "pi-sych-init", "pi-sych-mcp", "pi-sych-retro", "pi-sych-status", "pi-sych-sync"]);
...
test("bootstrapped worker starts with only the worker extension", async () => {
  ...
  await bootstrapWorkerAgentDir({ agentDir, packageRoot: process.cwd(), supervisorAgentDir });
  ...
  assert.equal(stderr, "");
  assert.deepEqual(commands.map((command) => command.name).filter((name) => name.startsWith("pi-sych-")), ["pi-sych-worker-status"]);
});
