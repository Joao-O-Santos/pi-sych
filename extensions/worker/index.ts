...
const resultParameters = Type.Object({
  ...
  role: Type.String(),
  status: Type.Union([Type.Literal("complete"), Type.Literal("failed"), Type.Literal("partial")]),
  summary: Type.String(),
  artifacts: Type.Array(Type.Object({ path: Type.String(), kind: Type.String() })),
...
export default function piSychWorker(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    description: "Submit the one immutable structured result for the active Pi Sych worker task.",
    ...
    async execute(_toolCallId, params) {
      ...
      if (!taskId || !runId || !resultPath) throw new Error("submit_artifact is available only inside an active Pi Sych worker dispatch");
      const result = validateWorkerResult({ ...params, taskId, runId }, { taskId, runId });
      await writeImmutableResult(resultPath, result);
      return { content: [{ type: "text", text: `Submitted immutable result for task ${taskId}.` }], details: { taskId, runId } };
    },
  });
