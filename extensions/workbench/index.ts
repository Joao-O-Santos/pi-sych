...
const evidenceProposalParameters = Type.Object({
	...
	title: Type.String(),
	kind: Type.Union([
		Type.Literal("empirical result"),
		Type.Literal("literature"),
		Type.Literal("official documentation"),
		Type.Literal("observed behaviour"),
	]),
	source: Type.String(),
...
const dispatchParameters = Type.Object({
	...
	inputs: Type.Optional(
		Type.Array(Type.Object({ path: Type.String(), purpose: Type.String() })),
	),
...
export class ApprovalLedger {
	...
	consume(absolutePath: string, content: string): boolean {
		if (this.approvedPlans.get(absolutePath) !== contentHash(content))
			return false;
		this.approvedPlans.delete(absolutePath);
		return true;
	}
...
function resolveProjectFile(
	cwd: string,
	inputPath: string,
): ProjectFileResolution | undefined {
...
function sendAnnotationFeedback(
	...
	void session
		...
		.catch((error: unknown) => {
			ctx.ui.notify(
				`Annotation failed: ${error instanceof Error ? error.message : String(error)}`,
				"error",
			);
		});
}
...
export default function piSychWorkbench(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		label: "Submit Plan",
		description:
			"Submit an existing Markdown plan for explicit user review. Approval does not start implementation automatically.",
		parameters: Type.Object({
			filePath: Type.String({
				description: "Project-relative path to a Markdown plan.",
			}),
		}),
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (
				!resolved ||
				![".md", ".mdx"].includes(extname(resolved.absolutePath).toLowerCase())
			) {
				return {
					content: [
						{
							type: "text",
							text: "Plan must be a non-empty Markdown file inside the project.",
						},
					],
			...
			if (!planContent.trim()) {
				return {
					content: [{ type: "text", text: "The plan file is empty." }],
					details: { approved: false },
					isError: true,
				};
			}
			...
			return {
				...
				details: {
					...
					filePath: resolved.projectRelative,
				},
			};
		},
	});
	pi.registerCommand("plannotator-annotate", {
		...
		async handler(args, ctx) {
			...
			if (!inputPath || !resolved) {
				ctx.ui.notify(
					"Usage: /plannotator-annotate <project-local-file>",
					"error",
				);
				return;
			}
			try {
				...
				sendAnnotationFeedback(
					...
					session,
					ctx,
					`Feedback for ${resolved.projectRelative}:`,
				);
			} catch (error) {
				ctx.ui.notify(
					`Unable to annotate ${inputPath}: ${error instanceof Error ? error.message : String(error)}`,
					"error",
				);
			}
		},
		...
		async handler(_args, ctx) {
			try {
				...
				sendAnnotationFeedback(
					...
					session,
					ctx,
					"Feedback on the last assistant message:",
				);
			} catch (error) {
				ctx.ui.notify(
					`Unable to annotate the last message: ${error instanceof Error ? error.message : String(error)}`,
					"error",
				);
			}
		},
	...
	pi.registerTool({
		...
		description:
			"Inspect canonical files and synchronization fingerprints, including changed artifacts and downstream files that need review.",
		parameters: Type.Object({}),
		async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
			const state = await inspectProjectSync(ctx.cwd);
			return {
				content: [{ type: "text", text: formatSyncSummary(state) }],
				details: state,
			};
		},
	});
	...
	pi.registerTool({
		...
		description:
			"Apply a drift resolution under an approved plan, including exact canonical/artifact content changes, then refresh SYNC.md fingerprints.",
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const planContent = planPath
				? readFileSync(planPath.absolutePath, "utf8")
				: "";
			if (!planPath || !approvals.consume(planPath.absolutePath, planContent)) {
				return {
					content: [
						{
							type: "text",
							text: "No unused approved plan matches approvalPlanPath; no files were changed.",
						},
					],
			...
			const state = await inspectProjectSync(ctx.cwd);
			const candidate = await buildReconciliationCandidate(
				state,
				params.finding,
				params.selectedResolution,
			);
	...
	pi.registerTool({
		...
		description:
			"Apply candidate contents atomically under a previously approved plan.",
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const written = await applyCandidate(
				ctx.cwd,
				{ ...candidateInput, requiresApproval: true } as CandidateReview,
				true,
			);
			return {
				content: [
					{
						type: "text",
						text: `Applied approved candidate files: ${written.join(", ")}`,
					},
				],
				details: { applied: true, written },
			};
		},
	});
	...
	pi.registerTool({
		...
		description:
			"Format a reviewable retrospective from explicit observations and actual verification records; never silently update canonical state.",
	...
	pi.registerTool({
		...
		description:
			"Format a reviewable evidence proposal that separates source claim, project interpretation, exact source, and limits.",
		parameters: evidenceProposalParameters,
		async execute(_toolCallId, params) {
			const proposal = buildEvidenceProposal(params);
			return {
				content: [{ type: "text", text: formatEvidenceProposal(proposal) }],
				details: proposal,
			};
		},
	});
	...
	pi.registerTool({
		...
		description:
			"Check EVIDENCE.md entries for required fields and unavailable declared sources without changing the file.",
		parameters: Type.Object({}),
		async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
			const challenges = await challengeEvidenceEntries(
				ctx.cwd,
				await readEvidence(ctx.cwd),
			);
			return {
				content: [{ type: "text", text: formatEvidenceChallenges(challenges) }],
				details: { challenges },
			};
		},
	});
	...
	pi.registerTool({
		...
		description:
			"Run one explicit executable-and-arguments verification contract and return actual exit code, bounded output, timestamps, and observed file changes.",
	...
	pi.registerCommand("pi-sych-retro", {
		description:
			"Create a review-gated retrospective proposal from explicit JSON input",
		handler: async (args, ctx) => {
			...
			try {
				ctx.ui.notify(
					buildRetrospectiveProposal(JSON.parse(args)).content,
					"info",
				);
			} catch (error) {
				ctx.ui.notify(
	...
	pi.registerCommand("pi-sych-status", {
		description:
			"Show the loaded Pi Sych package version and project synchronization status",
		handler: async (_args, ctx) => {
			try {
				const packageStatus = formatPackageStatus(getPackageStatus());
				const projectStatus = formatSyncSummary(
					await inspectProjectSync(ctx.cwd),
				);
	...
	pi.registerCommand("pi-sych-mcp", {
		description:
			"Show Pi Sych MCPorter dependency and explicit-configuration diagnostics",
		handler: async (_args, ctx) => {
			try {
	...
	pi.registerCommand("pi-sych-init", {
		description:
			"Start a focused, review-gated canonical project-state candidate",
		handler: async (args, ctx) => {
			try {
				...
				if (artifactPath) {
					ctx.ui.notify(
						formatCandidateReview(
							await artifactCandidateFromFile(ctx.cwd, artifactPath),
						),
						"info",
					);
					return;
				...
				ctx.ui.notify(
					`${question}\n\n${formatCandidateReview(candidate)}`,
					"info",
				);
			} catch (error) {
				ctx.ui.notify(
	...
	pi.registerCommand("pi-sych-drift", {
		description:
			"Report deterministic drift findings without reconciling files",
		handler: async (_args, ctx) => {
			try {
	...
	pi.registerCommand("pi-sych-sync", {
		...
		handler: async (_args, ctx) => {
			try {
				ctx.ui.notify(
					formatCandidateReview(await buildSyncCandidate(ctx.cwd)),
					"info",
				);
			} catch (error) {
				ctx.ui.notify(
