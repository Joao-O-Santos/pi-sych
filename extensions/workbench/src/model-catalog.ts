...
export function parseModelProfiles(value: unknown): ModelProfiles {
  ...
  if (catalog.default !== undefined) return { default: profileEntries(catalog.default, "default"), ...(catalog.profiles ? { profiles: Object.fromEntries(Object.entries(catalog.profiles).map(([name, entries]) => [name, profileEntries(entries, name)])) } : {}) };
  ...
  const resolveEntry = (entry: string): string => {
    ...
    if (typeof definition.ref !== "string" || !definition.ref.trim()) throw new Error(`Model '${entry}' must define a non-empty ref`);
    return definition.ref.trim();
  };
  const profiles = Object.fromEntries(Object.entries(catalog.profiles).map(([name, entries]) => [name, profileEntries(entries, name).map(resolveEntry)]));
...
export function loadModelProfiles(env: NodeJS.ProcessEnv = process.env): ModelProfiles {
  ...
  const path = env.PI_SYCH_MODEL_CATALOG ?? resolve(env.PI_CODING_AGENT_DIR ?? resolve(homedir(), ".config/pi"), "pi-sych/models.json");
  try {
    return parseModelProfiles(JSON.parse(readFileSync(path, "utf8")));
  } catch (error) {
    throw new Error(`Worker model catalog is unavailable or invalid at ${path}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
