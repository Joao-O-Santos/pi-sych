...
export const RETRYABLE_FAILURES = ["spawn-failure", "model-unavailable", "model-limit"] as const;
export type FailureClassification = (typeof RETRYABLE_FAILURES)[number]
  | "timeout"
  | "cancelled"
  | "schema-failure"
  | "tool-failure"
...
const MODE_TOOLS: Record<WorkerMode, readonly string[]> = {
  ...
  "full-host": ["read", "grep", "find", "ls", "edit", "write", "bash", "submit_artifact"],
};
...
export interface DispatchRequest {
  ...
  reviewLens?: string;
  modelProfile?: string;
  verification?: { commands: Array<{ executable: string; args: string[]; cwd?: string; expectedExitCode?: number }> };
  timeoutMs?: number;
  maxTurns?: number;
...
export function classifyModelError(message: string): FailureClassification | undefined {
  const normalized = message.toLowerCase();
  if (/unknown model|model.*(?:not found|unavailable|does not exist)|no model (?:was )?available/.test(normalized)) return "model-unavailable";
  if (/rate.?limit|too many requests|quota|usage limit|capacity|overloaded|\b429\b|\b529\b/.test(normalized)) return "model-limit";
  return undefined;
}
...
function timestamp(value: unknown, name: string): string {
  ...
  if (Number.isNaN(Date.parse(parsed))) throw new Error(`${name} must be an ISO timestamp`);
  return parsed;
}

function validateWorkerVerification(value: unknown, index: number): WorkerVerification {
...
export function validateDispatchRequest(value: unknown): DispatchRequest {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Dispatch request must be an object");
  ...
  const inputs = request.inputs === undefined ? undefined : (() => {
    ...
    return request.inputs.map((input, index) => {
      ...
      return { path: string(item.path, `inputs[${index}].path`), purpose: string(item.purpose, `inputs[${index}].purpose`) };
    });
  })();
  const verification = request.verification === undefined ? undefined : (() => {
    if (!request.verification || typeof request.verification !== "object" || Array.isArray(request.verification)) throw new Error("verification must be an object");
    const commands = (request.verification as Record<string, unknown>).commands;
    if (!Array.isArray(commands)) throw new Error("verification.commands must be an array");
    return { commands: commands.map((command, index) => {
      if (!command || typeof command !== "object" || Array.isArray(command)) throw new Error(`verification.commands[${index}] must be an object`);
      const current = command as Record<string, unknown>;
      const rawExpectedExitCode = current.expectedExitCode;
      const expectedExitCode = rawExpectedExitCode === undefined ? 0 : rawExpectedExitCode;
      if (typeof expectedExitCode !== "number" || !Number.isInteger(expectedExitCode)) throw new Error(`verification.commands[${index}].expectedExitCode must be an integer`);
      return {
        executable: string(current.executable, `verification.commands[${index}].executable`),
        args: commandArgs(current.args, `verification.commands[${index}].args`),
        ...(current.cwd === undefined ? {} : { cwd: string(current.cwd, `verification.commands[${index}].cwd`) }),
        expectedExitCode,
      };
    }) };
  })();
  const optionalPositiveInteger = (name: "timeoutMs" | "maxTurns"): number | undefined => {
    const number = request[name];
    if (number === undefined) return undefined;
    if (!Number.isInteger(number) || (number as number) <= 0) throw new Error(`${name} must be a positive integer`);
  ...
  return {
    ...
    ...(request.intendedWritePaths === undefined ? {} : { intendedWritePaths: strings(request.intendedWritePaths, "intendedWritePaths") }),
    ...(request.skills === undefined ? {} : { skills: strings(request.skills, "skills") }),
    ...(request.reviewLens === undefined ? {} : { reviewLens: string(request.reviewLens, "reviewLens") }),
    ...(request.modelProfile === undefined ? {} : { modelProfile: string(request.modelProfile, "modelProfile") }),
    ...(verification === undefined ? {} : { verification }),
    ...(optionalPositiveInteger("timeoutMs") === undefined ? {} : { timeoutMs: optionalPositiveInteger("timeoutMs") }),
    ...(optionalPositiveInteger("maxTurns") === undefined ? {} : { maxTurns: optionalPositiveInteger("maxTurns") }),
...
export function resolveSelectedSkillPaths(selectors: string[] = [], projectRoot: string, packageRoot: string): string[] {
  const userSkillRoot = resolve(process.env.PI_CODING_AGENT_DIR ?? resolve(homedir(), ".config/pi"), "skills");
  return [...new Set(selectors.map((selector) => {
    if (selector.includes("/") || selector.endsWith(".md")) {
      ...
      const path = existsSync(selected) && statSync(selected).isDirectory() ? resolve(selected, "SKILL.md") : selected;
...
export function toolsForRequest(request: Pick<DispatchRequest, "mode" | "remoteResearch">): readonly string[] {
  return [...new Set([...MODE_TOOLS[request.mode], ...(request.remoteResearch ? ["mcporter"] : [])])];
}

export function resolveModelProfile(profiles: ModelProfiles, requested?: string): string[] {
  const candidates = requested ? profiles.profiles?.[requested] : profiles.default;
  if (!Array.isArray(candidates) || candidates.length === 0 || candidates.some((model) => typeof model !== "string" || !model.trim())) {
...
function formatVerificationCommands(request: DispatchRequest): string {
  ...
  if (!commands.length) return "Supervisor verification contract: none specified";
  return [
    "Supervisor verification contract (runs after artifact submission):",
    ...commands.map((command, index) => `- [${index + 1}] ${JSON.stringify({
...
export async function writeImmutableResult(path: string, result: WorkerResult): Promise<void> {
  ...
  const handle = await open(path, "wx", 0o600).catch((error: NodeJS.ErrnoException) => {
    if (error.code === "EEXIST") throw new Error("Worker result is immutable and has already been submitted");
    throw error;
  });
  try {
...
export function validateWorkerResult(value: unknown, identity: TaskIdentity): WorkerResult {
  ...
  if (string(result.taskId, "taskId") !== identity.taskId || string(result.runId, "runId") !== identity.runId) throw new Error("Worker result identity does not match dispatch");
  const status = string(result.status, "status");
  if (status !== "complete" && status !== "failed" && status !== "partial") throw new Error(`Invalid worker result status: ${status}`);
  const artifacts = result.artifacts === undefined ? [] : (() => {
    ...
    return result.artifacts.map((artifact, index) => {
      if (!artifact || typeof artifact !== "object" || Array.isArray(artifact)) throw new Error(`artifacts[${index}] must be an object`);
      const item = artifact as Record<string, unknown>;
      return { path: string(item.path, `artifacts[${index}].path`), kind: string(item.kind, `artifacts[${index}].kind`) };
    });
  })();
  return {
    schemaVersion: 1,
...
export async function readWorkerResult(path: string, identity: TaskIdentity): Promise<WorkerResult | undefined> {
...
export class MutationLock {
  ...
  async acquire(identity: TaskIdentity): Promise<void> {
    ...
    const handle = await open(this.path, "wx", 0o600).catch((error: NodeJS.ErrnoException) => {
      if (error.code === "EEXIST") throw new Error("A Pi Sych mutating worker is already active for this project");
      throw error;
    });
    try {
  ...
  async release(identity: TaskIdentity): Promise<void> {
    ...
    if (owner.taskId !== identity.taskId || owner.runId !== identity.runId) throw new Error("Mutation lock ownership does not match this worker run");
...
async function gitCapture(projectRoot: string, args: string[]): Promise<GitCapture | undefined> {
...
async function gitOutput(projectRoot: string, args: string[]): Promise<string | undefined> {
...
async function ensureRuntimeGitExcluded(projectRoot: string): Promise<void> {
  const ignored = await gitCapture(projectRoot, ["check-ignore", "-q", "--no-index", ".pi-sych/probe"]);
  if (!ignored || ignored.code === 128 || ignored.code === null || ignored.code === 0) return;
  const gitPath = (await gitOutput(projectRoot, ["rev-parse", "--git-path", "info/exclude"]))?.trim();
  if (!gitPath) return;
  ...
  try {
    ...
    throw new Error(`Unable to exclude .pi-sych runtime state from Git; add '.pi-sych/' to .gitignore (${reason})`);
  }
}

async function gitStatusEntries(projectRoot: string): Promise<GitStatusEntry[] | undefined> {
  const output = await gitOutput(projectRoot, ["status", "--porcelain=v1", "-z", "-uall"]);
...
export async function projectChangeSnapshot(projectRoot: string): Promise<ProjectChangeSnapshot> {
  ...
  for (const entry of entries) {
    let content = "missing";
    try {
      content = createHash("sha256").update(await readFile(resolve(projectRoot, entry.path))).digest("hex");
    } catch {
      // A missing file is itself part of the Git state.
    }
    values.set(entry.path, `${entry.status}:${content}`);
  }
  const tree = (await gitOutput(projectRoot, ["rev-parse", "HEAD^{tree}"]))?.trim();
  return { values, ...(tree ? { tree } : {}), available: true };
}

async function treeChanges(projectRoot: string, before?: string, after?: string): Promise<string[]> {
...
export async function changesSince(
  ...
  const committed = new Set(await treeChanges(projectRoot, before.tree, after.tree));
  for (const path of committed) paths.add(path);
  return [...paths].filter((path) => before.values.get(path) !== after.values.get(path) || committed.has(path));
}

export async function observedProjectChanges(projectRoot: string): Promise<string[]> {
  return (await gitStatusEntries(projectRoot))?.map((entry) => entry.path) ?? [];
}

export function unexpectedChanges(observed: string[], intended: string[] = []): string[] {
  const allowed = new Set(intended);
  return observed.filter((path) => !allowed.has(path));
}

export const launchPiWorker: WorkerLauncher = async (spec) => new Promise((resolvePromise) => {
  const args = [
    "--mode", "json", "--print", spec.prompt,
    "--no-session", "--no-extensions", "--extension", resolve(spec.packageRoot ?? PI_SYCH_PACKAGE_ROOT, "extensions/worker/index.ts"),
    ...(spec.extraExtensionPaths ?? []).flatMap((path) => ["--extension", path]),
    "--no-skills", "--no-prompt-templates", "--no-themes", "--no-context-files", "--no-approve",
    "--tools", toolsForRequest(spec.request).join(","),
    "--model", spec.model,
  ];
  for (const skill of resolveSelectedSkillPaths(spec.request.skills, spec.projectRoot, spec.packageRoot ?? PI_SYCH_PACKAGE_ROOT)) args.push("--skill", skill);
  let stdout = "";
  let stderr = "";
  let timedOut = false;
  ...
  const child = spawn(process.env.PI_SYCH_PI_BIN ?? "pi", args, {
    ...
    env: {
      ...
      ...(spec.request.remoteResearch ? { MCPORTER_CONFIG: process.env.PI_SYCH_MCPORTER_CONFIG ?? resolve(process.env.HOME ?? "~", ".config/pi-sych/mcp/mcporter.json") } : {}),
    },
  ...
  child.stdout.on("data", (chunk) => {
    ...
    while (eventBuffer.includes("\n")) {
      ...
      try {
        const event = JSON.parse(line) as { type?: string; finalError?: string; message?: { role?: string; stopReason?: string; errorMessage?: string } };
        if (
          event.type === "message_end"
          && event.message?.role === "assistant"
          && event.message.stopReason === "error"
        ) {
          classification ??= classifyModelError(event.message.errorMessage ?? "");
        }
        if (event.type === "auto_retry_end" && event.finalError) classification ??= classifyModelError(event.finalError);
        if (event.type === "turn_end") {
          turnCount += 1;
          if (spec.request.maxTurns !== undefined && turnCount >= spec.request.maxTurns) {
  ...
  const timeout = setTimeout(() => terminate("timeout"), spec.request.timeoutMs ?? 120_000);
  child.once("error", (error) => {
    settled = true;
    clearTimeout(timeout);
    if (graceTimer) clearTimeout(graceTimer);
    spec.signal?.removeEventListener("abort", abortHandler);
    resolvePromise({ exitCode: null, stdout, stderr: bounded(`${stderr}${error.message}`), timedOut, classification });
  });
...
export async function dispatchWorker(options: {
  ...
  for (const input of request.inputs ?? []) {
    const path = isAbsolute(input.path) ? input.path : resolve(options.projectRoot, input.path);
    try {
  ...
  const resultPath = resolve(options.projectRoot, ".pi-sych", "runs", `${identity.taskId}-${identity.runId}.json`);
  ...
  try {
    for (const model of models.slice(0, 2)) {
      ...
      if (result && request.verification?.commands.length) {
        ...
        const reports = await runVerificationContract(request.verification.commands, options.projectRoot);
        result = { ...result, verification: [...result.verification, ...reports] };
        ...
        if (failed.length) {
          ...
          verificationOutput = failed.map((report) => report.stderrTail || report.stdoutTail).filter(Boolean).join("\n");
        }
      }
      const finalSnapshot = await projectChangeSnapshot(options.projectRoot);
      const observed = await changesSince(baseline, finalSnapshot, options.projectRoot);
      const changeInspection = baseline.available && finalSnapshot.available ? "available" : "unavailable";
      const unexpected = unexpectedChanges(observed, request.intendedWritePaths);
      if (result) {
        const verifiedResult = {
          ...result,
          observedChanges: changeInspection === "available" ? observed : result.observedChanges,
        };
      ...
      const classification: FailureClassification = schemaError
        ...
        : lastOutcome.classification
          ?? (lastOutcome.timedOut ? "timeout" : lastOutcome.exitCode === null ? "spawn-failure" : "incomplete");
      const retryable = RETRYABLE_FAILURES.includes(classification as (typeof RETRYABLE_FAILURES)[number]);
      if (!retryable || observed.length > 0) {
        ...
        return {
          ...
          failure: {
            ...
            stderrTail: bounded(schemaError ? `${schemaError}\n${lastOutcome.stderr}` : lastOutcome.stderr),
...
export async function saveSyntheticWorkerResult(path: string, result: WorkerResult): Promise<void> {
