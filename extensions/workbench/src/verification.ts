...
export function validateVerificationCommand(
	...
	if (!value || typeof value !== "object" || Array.isArray(value))
		throw new Error("Verification command must be an object");
	const command = value as Record<string, unknown>;
	const expectedExitCode =
		command.expectedExitCode === undefined ? 0 : command.expectedExitCode;
	if (
		typeof expectedExitCode !== "number" ||
		!Number.isInteger(expectedExitCode)
	)
		throw new Error("expectedExitCode must be an integer");
	...
	const execution = await new Promise<{
		...
		child.once("error", (error) => {
			...
			resolvePromise({
				exitCode: null,
				stdout,
				stderr: tail(`${stderr}${error.message}`),
			});
		});
...
export async function runVerificationContract(
	...
	for (const command of commands)
		reports.push(await runVerification(command, defaultCwd));
	return reports;
}
