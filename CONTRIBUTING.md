...
3. Configure npm’s **Trusted Publisher** for GitLab CI/CD once: namespace `Joao-O-Santos`, project `pi-sych`, CI file `.gitlab-ci.yml`.
4. Create an annotated tag whose name exactly matches `v` plus `package.json`’s version, then push it to GitLab.

The GitLab tag pipeline reruns the checks and publishes with npm provenance through GitLab OIDC. It does not use an npm token stored in this repository. Do not create a release tag until the npm account and Trusted Publisher are configured.
