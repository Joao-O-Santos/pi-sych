# Copying and provenance

Pi Sych is distributed under the MIT license in [LICENSE](LICENSE). That
license covers Pi Sych's original code and documentation except where a file
identifies a separate provenance or notice below.

## Adapted make-it-stop material

[Makefile](Makefile) is adapted from `make-it-stop` commit
`4784c8b507c3c8748c97d7d14e25facf6174af85` and identifies that adaptation in
its SPDX header. Its BSD-3-Clause notice is retained at
[LICENSES/BSD-3-Clause](LICENSES/BSD-3-Clause).

[config.mk](config.mk) is adapted site configuration from the same project and
identifies CC0-1.0 in its SPDX header. The CC0 notice is retained at
[LICENSES/CC0-1.0](LICENSES/CC0-1.0). Those notices describe the adapted
components; they do not replace the repository's MIT license for Pi Sych as a
whole.

## Influence without copying

[Ponytail](https://github.com/DietrichGebert/ponytail) is MIT-licensed. It
influenced Pi Sych's coding preferences only. No Ponytail code or prose is
copied into Pi Sych, so its MIT terms are not a license for a copied component.

Eric S. Raymond's *The Art of Unix Programming* is licensed CC BY-ND 1.0.
Pi Sych cites it as an influence and paraphrases ideas rather than adapting or
reproducing its prose. See the book and license at
<http://www.catb.org/~esr/writings/taoup/html/>.

The literature and platforms listed in [docs/attribution.md](docs/attribution.md)
are attribution for intellectual influence, empirical context, platforms, or
integrations. They are not a license grant for Pi Sych, and their own works and
packages retain their own terms.
